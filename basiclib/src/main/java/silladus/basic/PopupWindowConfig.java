...
public class PopupWindowConfig {
    ...
    public void showAt(@NonNull View anchorView) {
        if (windowPos == null) {
            // 设置好参数之后再show
            windowPos = calculatePopWindowPos(anchorView, mPopupWindow.getContentView(), mPopupWindow.getWidth() - left + right);
        }
    ...
    private static int[] calculatePopWindowPos(@NonNull View anchorView, @NonNull View contentView, int windowWidth) {
...
//        int windowWidth = contentView.getMeasuredWidth();
    ...
    public PopupWindowConfig setMargin(int left, int top, int right) {
        this.left = left;
        this.top = top;
        this.right = right;
        return this;
